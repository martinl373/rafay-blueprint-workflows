...
function __delete_workspace() {
    [[ -d "${workspace}" ]] &&  echo rm -rf "${workspace}"
}
...
if [[ -z $(yq '.spec.artifact.artifact.repository // ""' "${addon_spec_file}") ]]; then
    ...
    if [[ -d "${chart_path}" ]]; then
        helm package --dependency-update --destination "${workspace}" $(dirname "${addon_spec_file}")/"${chart_path}"
...
[[ ! -z "${addon_name:-}" ]] && yq -i '.metadata.name = "'${addon_name}'"' "${addon_spec_file}"
[[ ! -z "${addon_project:-}" ]] && yq -i '.metadata.project = "'${addon_project}'"' "${addon_spec_file}"
[[ ! -z "${addon_namespace:-}" ]] && yq -i '.spec.namespace = "'${addon_namespace}'"' "${addon_spec_file}"
[[ ! -z "${addon_version:-}" ]] && yq -i '.spec.version = "'${addon_version}'"' "${addon_spec_file}"

echo "-- Rendered Spec File --"
cat ${addon_spec_file}
echo "----------------------"

RCTL_API_KEY="${rafay_api_key}" \
RCTL_API_SECRET="${rafay_api_key}" \
rctl create addon version --v3 -f "${addon_spec_file}"